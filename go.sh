...
--build-arg AGENCYNAME="srtk" \
--build-arg GTFS_URL="https://github.com/azizBfth/gtfs-file/raw/main/gtfs.zip?raw=true" .
...
docker run -d --name transitclock-server-instance --rm --link transitclock-db:postgres -e PGPASSWORD=$PGPASSWORD  -v ~/logs:/usr/local/transitclock/logs/ -v ~/ehcache:/usr/local/transitclock/cache/ -p 8080:8080 transitclock-server  start_transitclock.sh
